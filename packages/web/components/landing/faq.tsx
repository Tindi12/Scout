...
export function FAQ() {
  return (
    <section id="faq" className="relative px-4 py-14 sm:px-6 sm:py-20 lg:px-12 lg:py-28">
      <div className="mx-auto max-w-3xl">
        ...
        <Accordion.Root type="single" collapsible className="space-y-2.5 sm:space-y-4">
          {FAQS.map((item, i) => (
            <Accordion.Item
              key={item.q}
              value={`item-${i}`}
            ...
            >
              <Accordion.Header>
                <Accordion.Trigger className="group flex w-full items-center justify-between gap-3 px-4 py-4 text-left sm:gap-4 sm:px-7 sm:py-6">
                  <span className="font-headline text-[14px] font-medium leading-snug text-white sm:text-[17px]">
                    {item.q}
                  </span>
                  <ChevronDown
              ...
              <Accordion.Content className="overflow-hidden data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down motion-reduce:data-[state=closed]:animate-none motion-reduce:data-[state=open]:animate-none">
                <div className="px-4 pb-4 font-body text-[14px] leading-relaxed text-[#A1A1AA] sm:px-7 sm:pb-6 sm:text-[15px]">
                  {item.a}
                </div>
              </Accordion.Content>
            </Accordion.Item>

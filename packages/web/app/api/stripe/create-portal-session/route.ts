 ...
 * with the shared internal secret. The upstream returns a Stripe billing portal URL
 * the client redirects to so users can manage / cancel their subscription.
 */
export async function POST() {
  ...
  try {
    upstream = await fetch(`${getApiBaseUrl()}/stripe/create-portal-session`, {
      method: 'POST',
      headers: {
